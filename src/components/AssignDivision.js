...
/**
 * Component that allows a logged in user to assign a selected user to
 * a selected division.
 * @param {*} props 
 * @returns 
 */
export function AssignDivision(props) {
    ...
    /**
     * Assigns a selected user to a specified division. The logged in user can only
     * assign a selected user to a division that they themselves belong to.
     */
    async function assignUserDivision() {
        ...
        if(selectedOrgUnit === '<Organisational Unit>' || selectedDivision === '<Division>') {
            setShowPopup(true);
            setPopupMessage('Cannot assign user division. Please select an organisational unit and division.');
            setPopupType('Error');
        }
        else {

            // If the selected organisational unit and selected division are not set to their
            // default values, then we proceed with the assignment of the selected user to
            // a division.

            try {

                // Sets the body for the assign division POST request.
                const updateBody = {
                ...
                // Assigns the selected user to the selected division.
                const updateResponse = await fetch('/assign-division', {
                    method: 'POST',
                    headers: {
                ...
                // The assign division response is converted to a JSON object.
                const updateResult = await updateResponse.json();

                // If the assign division JSON object has an error key, then
                ...
                // Fetches the list of all users after assigning the selected
                // user to the selected division.
    ...
    if(token === '') {
        ...
        return(
            <div className='change-role-content'>
                <h1>Assign Division</h1>
                <p>You are not logged in, and thus you are not authorized to assign user divisions.</p>
            </div>
        );
    }
    else {
        ...
        return(
            <div className='change-role-content'>
                ...
                {/*
                    ...
                    cannot be assigned to the selected division.
                */}
                ...
                <div className='new-block'>You are about to assign the following user to a division:</div>
                <div><b>Username:</b> {selectedUser.username}</div>
                <div><b>Role:</b> {selectedUser.role}</div>

                <div className='new-block'>Choose the organisational unit and division to which you would like to assign the user:</div>
                ...
                {/* Allows the user to assign a selected user to a selected division. */}
                <button onClick={assignUserDivision} className='custom-btn'>Assign Division</button>
            </div>
        );
    }
}